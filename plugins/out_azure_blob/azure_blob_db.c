...
static int prepare_stmts(struct flb_sqldb *db, struct flb_azure_blob *ctx)
{
    ...
    /* insert */
    ret = sqlite3_prepare_v2(db->handler, SQL_INSERT_BLOB_FILE, -1,
                             &ctx->stmt_insert_file, NULL);
    if (ret != SQLITE_OK) {
        flb_plg_error(ctx->ins, "cannot prepare SQL statement: %s",
                      SQL_INSERT_BLOB_FILE);
        return -1;
    }

    /* delete */
    ret = sqlite3_prepare_v2(db->handler, SQL_DELETE_BLOB_FILE, -1,
                             &ctx->stmt_delete_file, NULL);
    if (ret != SQLITE_OK) {
        flb_plg_error(ctx->ins, "cannot prepare SQL statement: %s",
                      SQL_DELETE_BLOB_FILE);
        return -1;
    }

    /* get */
    ret = sqlite3_prepare_v2(db->handler, SQL_GET_BLOB_FILE, -1,
